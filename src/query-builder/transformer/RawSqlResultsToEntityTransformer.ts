...
export class RawSqlResultsToEntityTransformer {
    ...
    protected transformColumns(rawResults: any[], alias: Alias, entity: ObjectLiteral, metadata: EntityMetadata): boolean {
        ...
        metadata.columns.forEach(column => {
            ...
            if (this.expressionMap.queryType === "select" && !this.expressionMap.selects.find(select => select.selection === alias.name || select.selection === alias.name + "." + column.propertyPath))
                return;
    ...
    private createValueMapFromJoinColumns(relation: RelationMetadata, parentAlias: string, rawSqlResults: any[]): ObjectLiteral {
        ...
        return columns.reduce((valueMap, column) => {
            rawSqlResults.forEach(rawSqlResult => {
                if (relation.isManyToOne || relation.isOneToOneOwner) {
                    valueMap[column.databaseName] = rawSqlResult[DriverUtils.buildColumnAlias(this.driver, parentAlias, column.databaseName)];
                } else {
                    valueMap[column.databaseName] = rawSqlResult[DriverUtils.buildColumnAlias(this.driver, parentAlias, column.referencedColumn!.databaseName)];
                }
            });
            return valueMap;
        }, {} as ObjectLiteral);
